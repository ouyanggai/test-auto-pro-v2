...
// 这是设计要求的结果，不是缺陷。
// 2026-09-05 F-016 T09 首次真实写实测登记：在计划 11（oyg00）路径 1121 上，
// 提交动作经七阶段全流程发出唯一一次写请求，目标受理（isSuccess=true，实例 caf2046d896f477c81c819153fc7d52f，
// status=run，trace 5ad3ab3454cce08d，运行 8）。当时核验重读读不到实例，按矩阵判不确定并停止。
// 2026-09-05 F-018 收口补记：那次「不确定」的根因是工具自己的读取口径——按实例精确复查带了公司业务关联
// 过滤，把本工具发起的实例整条排除（语义清单第 19 条）。修掉后同一条路径的提交动作实测判为
// 确定成功（运行 13，实例 7bcf0c29f1054ba0bed5cf367ba2f2d2，trace 0715f0cdf7cc2700，重读结论 advanced），
// 因此提交是真正跑通过的动作，登记继续有效。
// 同意（approve）仍未在真实目标上执行过，不登记：该账号手上没有任何审批待办（实测 169 条待办全是
// 草稿/驳回/撤回），且对已有实例动手的执行接线属 F-019。其余动作继续只给提醒不阻塞。
