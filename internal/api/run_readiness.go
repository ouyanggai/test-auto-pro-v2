...
type RunReadinessService interface {
	...
	PlanReadiness(ctx context.Context, planID uint64) (model.PlanRunReadiness, error)
}
...
func handlePlanRunReadiness(readiness RunReadinessService) http.HandlerFunc {
	return func(response http.ResponseWriter, request *http.Request) {
		...
		result, err := readiness.PlanReadiness(request.Context(), planID)
		if err != nil {
			writeRunReadinessError(response, err)
			return
		}
...
func (unavailableRunReadinessService) PlanReadiness(context.Context, uint64) (model.PlanRunReadiness, error) {
