...
- 当前功能：F-015 成功断言与运行准备（`ready_for_manual`）；F-013 与 F-014 已由用户明确验收为 `accepted`。
- F-015 已完成：迁移 024 与 `test_path_success_assertions` 表（独立 revision，与路径配置修订解耦）；
  断言候选与取值校验只依据真实结构与目标真实状态八个取值；纯判定包 `internal/engine/assert`
  给出成立、不成立、无法判定三值且"无法判定"绝不合并进"不成立"；运行准备聚合十类阻塞与两类提醒并删除
  `IsExecutionPathRunnable`；三个只读接口；前端成功断言卡片与只读运行准备面板（阻塞可点击定位、阻塞与提醒分区）；
  真实 MySQL 与真实目标集成测试、两个契约脚本与 `test/run-f015.sh`。
- F-015 关键实测结论：真实目标模板一个 `end` 类型节点都没有（只有 start/manual/condition/common/empty/synergy），
  流程直接终止在没有后继的审批节点上。结束节点已改为按结构事实判断，否则成功断言在真实数据上无法配置。
...
- F-012（上一切片）状态：`implementing`。用户人工复验未通过，正在把 FormMaking 从直接挂载改为目标 `OtherSteps2` 完整宿主渲染，并收敛为单一数据回显桥接。
...
- 下一步：等用户按清单人工验收 F-014；未获明确验收前不进入 `accepted`。F-012 由另一线程返工中，F-013 已验收完成。
