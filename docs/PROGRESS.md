...
- 当前功能：F-015 运行前检查（`ready_for_manual`，人工复审 5 个问题已修复并重新验证）；F-013 与 F-014 已由用户明确验收为 `accepted`。
...
- 下一步：等用户按清单人工验收 F-015（`ready_for_manual`），未获明确验收前不进入 `accepted`；F-016 已获批准并由并行线程 `implementing` 中，F-017 计划停在 `awaiting_approval`，必须等 F-016 实施完成并由用户验收后才能进入 `implementing`，其第一个任务是按 F-016 的实际实现校准计划文件。F-012 由另一线程返工中，F-013 与 F-014 已验收完成。
