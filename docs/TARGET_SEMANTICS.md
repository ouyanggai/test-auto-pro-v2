...
- 会签节点（如「会签，满足 3 人」）在同一节点上等待多名处理人各自审批后才前进：工具以代理树配置人员之一完成一次真实同意后，实例仍停留在该节点，等待其余处理人（运行 72 第 9 步后的门禁事实：实例待办仍在「审核人3」）。编译场景每节点只生成一次审批动作，因此会签节点的多人审批是当前执行模型的已知缺口；该缺口阻塞路径 1 在审核人3之后的推进。
