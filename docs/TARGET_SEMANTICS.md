...
| 2 | 幂等与重复提交 | 已勘定（F-014） |
| 3 | 条件求值 | 未开始 |
| 4 | 演员解析 | 勘定中（2026-09-10 F-028 验收实测：待办按用户过滤、固定人员经代理树发现；详见第 4 节） |
| 5 | 回退语义 | 未开始 |
| 6 | 取回语义 | 未开始 |
| 7 | 转发语义 | 未开始 |
| 8 | 加签与移交语义 | 已勘定（加签协议已实现，待真实写验证） |
| 9 | 会签与并行语义 | 勘定中（2026-09-10 实测：会签节点需同节点多人审批才前进，单次审批后流程原地等待；详见第 9 节） |
| 10 | 附件与富文本 | 未开始 |
| 11 | 表单权限与可见性 | 未开始 |
| 12 | 子流程语义 | 未开始 |
...
状态：勘定中（2026-09-10 单点实测）。

- 会签节点（如「会签，满足 3 人」）在同一节点上等待多名处理人各自审批后才前进：一名处理人同意后，实例仍停留在该节点，其余处理人的待办依旧存在（currentAuditUserInfo 的 userList 随审批进度递减，运行 72 第 9 步后实测）。
- 会签中间态的核验口径：按节点对照「待办是否清空」会把部分完成误判成写未生效；正确口径是按执行人过滤的已办——本演员在本节点的已完成任务出现即本次审批已被目标记录（工具已在核验中实现该口径）。
- 工具的执行模型：同意步骤成功后若实例仍停在本节点，用新发现的当前处理人原步重跑（每人一次独立七阶段与独立落账），直至实例前进（2026-09-11 实现；端到端推进受目标会话互相踢的易变性限制，见第 1.3 节与会话限制条目）。

## 13. 超时与跳过语义

状态：勘定中（2026-09-10 F-028 验收实测，正式证据块待补）。
