...
- 2026-09-04 `implementing` -> `ready_for_manual`：T01 至 T08 全部实施完成，`./test/run-f014.sh` 本机实跑通过。
  ...
  一处如实记录的不稳定：连续运行 `./test/run-f014.sh` 四次，其中一次在只读集成阶段失败，
  随后两次连续通过。失败发生在连真实目标的那一步，未捕获到具体断言（当时只看了输出尾部），
  可复现的最可能原因是真实目标登录或模板列表的瞬时失败。按计划要求这类用例不得静默跳过，
  因此没有加重试掩盖它；用例的失败信息已带上目标返回的原始内容，下次出现可直接定位。
  待人工验收；F-015 未开始。
