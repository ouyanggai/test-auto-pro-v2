...
| 配置项 | 用途 | 当前依据与状态 |
| --- | --- | --- |
| `TARGET_API_GATEWAY` | 目标平台 base URL | 必需；沿用 V1 环境变量名，只在后端启动环境提供 |
| `TARGET_LOGIN_PASSWORD` | 目标测试环境服务端登录密码 | 必需且敏感；页面只输入账号，真实值只在后端启动环境提供 |
| `TARGET_LOGIN_AES_KEY` | 登录密码加密 key | V1 配置模型已有该项；必须由服务端环境提供，不硬编码、不记录日志 |
| `TARGET_LOGIN_CODE` | 目标环境登录 `code` | 必需且敏感；沿用 V1 请求字段，真实值只在后端启动环境提供 |
...
环境协议与非敏感默认约定已经用户批准沿用 V1。实际连接、登录敏感值仍只从 Go 后端运行时环境变量取得，不写入仓库。
...
- `./test/run-f002.sh`：通过。
- Go 定向测试：后端单元 8 项、API 契约 4 项、假目标集成 6 项顶层用例全部通过；集成子用例覆盖 HTTP/三类业务会话失效和一次重放上限。
- 前端单元：6 项全部通过；结构契约脚本通过。
