...
V1 中有日志把 SID 当字段输出的做法，不符合本功能安全要求，明确不复制。V1 测试环境还使用了固定密码和验证码约定；该约定是否仍适用于当前目标环境必须由用户确认，不能直接当成新项目事实。
...
新项目 F-002 采用进程环境变量，不接入 V1 的 Nacos 或配置中心。根目录的后端前台开发脚本继承启动终端的环境变量；仓库只允许提交不含值的配置说明，真实值不得写入 Git。

| 配置项 | 用途 | 当前依据与状态 |
| --- | --- | --- |
| `TARGET_API_GATEWAY` | 目标平台 base URL | V1 静态配置与环境变量已使用该名称；F-002 必需，当前项目尚未配置，实施前需用户提供 |
| `TARGET_LOGIN_PASSWORD` | 目标测试环境服务端登录密码 | 页面只输入账号，因此密码必须来自服务端安全配置；是否可沿用统一测试密码约定需用户确认 |
| `TARGET_LOGIN_AES_KEY` | 登录密码加密 key | V1 配置模型已有该项；必须由服务端环境提供，不硬编码、不记录日志 |
| `TARGET_LOGIN_CODE` | 目标环境登录 `code` | V1 测试环境使用固定约定；当前环境是否仍适用需用户确认 |
| `TARGET_PLATFORM_CODE` | 登录与普通目标请求的平台代码 | V1 默认 `200001`；当前环境值需用户确认后配置 |
| `TARGET_TEMPLATE_PLATFORM_CODES` | 模板列表的顶层平台过滤 | V1 已核实为 `200001,999999`；当前环境是否沿用需用户确认 |
| `TARGET_CUSTOMER_CODE` | 首次登录所需租户代码 | V1 可由配置提供，登录后也能从用户/公司摘要取得；当前环境是否允许首次为空需用户确认 |
| `TARGET_SESSION_TTL` | 进程内会话绝对过期时间 | 可选；建议默认 `8h`，依据 V1，目标更早失效时由一次安全重登处理 |
...
- 默认绝对 TTL 建议 8 小时，可由 `TARGET_SESSION_TTL` 缩短；不主动为每次列表请求增加 `checkLoginStatus` 往返。
...
- `items[]`：`id`、`flowName`、`code`、`groupName`、`flowStatus`、`statusText`、`typeName`、`updateDate`、`createDate`、`remark`、`flowCreateType`。
...
| 必需服务端配置缺失 | `503 TARGET_CONFIG_MISSING` | 明确提示“目标环境尚未配置”，不进入已验证态 | 工具配置 |
| 目标拒绝登录 | `401 TARGET_LOGIN_REJECTED` | 账号保持未验证，显示可理解的登录失败提示 | 目标平台拒绝 |
...
| 目标分页字段或响应结构异常 | `502 TARGET_RESPONSE_INVALID` | 保留当前已选兼容状态，显示“目标数据格式异常，可重试” | 工具适配，待核实目标变化 |
| 目标网络失败或 5xx | `502 TARGET_UNAVAILABLE` | 显示可重试错误，不回退 mock | 目标平台或链路，暂按工具问题处理 |
| 目标请求超时 | `504 TARGET_TIMEOUT` | 结束 loading，允许重试 | 工具链路，待核实目标状态 |
...
- [ ] 用户输入非空真实账号并点击验证后，后端真实登录；成功摘要准确，失败不伪装为已验证。
- [ ] SID 只存在于 Go 后端进程，浏览器响应、前端状态/存储和应用日志均无法取得 SID。
- [ ] 同账号并发登录被去重；缓存按账号隔离并遵守 TTL；会话失效最多重登和重放一次。
- [ ] “新发起”读取并分页展示账号可见真实模板，字段与请求参数符合已核实证据。
- [ ] “已发”“待发”分别读取真实实例，字段、状态和请求参数符合各自参考来源，不复用错误契约。
- [ ] 搜索防抖、取消/迟到响应保护、分页追加、去重以及加载/空/错误/到底状态实际可操作。
- [ ] 编辑账号或切换来源会清空不兼容选择，不会被旧请求回写；候选区几何和自动引导不回退。
- [ ] 目标请求全程只读；不保存计划、不创建数据库记录、不修改目标平台、不进入路径选择。
- [ ] F-002 当前范围测试、Go build、前端类型检查和生产构建通过。
- [ ] 文档状态更新为 `ready_for_manual`，列出真实环境人工步骤并停止，不开始 F-003。

## 当前范围自动验证

所有新增测试必须位于根目录 `test/`，只运行 F-002 相关测试：

- `test/unit/backend/`：配置必填与脱敏、TTL、账号隔离、同账号并发去重、一次重登上限、登录/模板/实例 DTO 映射。
- `test/contracts/`：三个 API 的请求、成功字段、分页、稳定错误码，以及任何响应均不含 SID。
- `test/integration/`：使用 `httptest.Server` 模拟目标平台，覆盖登录、三类列表、空列表、HTTP/业务会话失效、401 后一次重登、超时、取消、坏分页和目标 5xx；不得依赖真实目标环境。
- `test/unit/frontend/`：搜索防抖、request version、账号/来源切换忽略旧响应、分页追加去重、到底和错误恢复。
...
用户需要在本地启动终端以环境变量提供真实配置，并另行准备可人工使用的最小账号；任何值都不写入仓库、文档或测试 fixture。
...
1. 在后端启动终端注入已确认的目标环境变量，再用根 `package.json` 中实际的前台开发脚本分别启动后端和前端。
...
5. 选择“新发起”，核对真实模板的搜索、首屏、滚动追加、选中、空结果和到底状态。
...
以下是会阻塞实现或改变安全/行为边界的最少问题：

1. 当前目标环境的 `TARGET_API_GATEWAY` 应取哪个 base URL，开发机是否可直接访问？
2. 当前环境是否仍允许“页面只输入账号、服务端使用统一测试密码和固定 code”的登录约定？若不是，密码/验证码或单点登录凭证应由哪个服务端安全来源按账号取得？
3. 当前环境的 `TARGET_PLATFORM_CODE`、模板过滤平台代码、首次登录 `TARGET_CUSTOMER_CODE` 是否分别沿用 V1 的 `200001`、`200001,999999`、可为空约定？登录 AES key 由哪个安全环境变量提供？
4. 可用于人工验收的最小账号与最简单流程是什么；该账号是否至少有一条可见模板、已发实例和待发实例？这些值只在验收现场提供，不写入仓库。
