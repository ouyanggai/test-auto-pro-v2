...
## 十九、2026-08-24 人工验收反馈返工台账

用户在人工验收中确认公司、部门、姓名与三个外部候选字段未按计划发起人完整回填，且生成结果提示占用表单工作区高度。本轮已将 F-010 与 F-010R 退回 `implementing`；F-009 继续保持 `ready_for_manual`。

本轮完成标准：

1. 以计划发起人的已验证运行时会话调用真实公司目录，完整生成公司、部门、姓名的 ID 与名称，并通过 FormMaking/Vue iframe 原位回显；单条生成和 F-009 批量准备复用同一身份投影。
2. 盘点实际注册的自定义组件。只为已由 `GroupApproveManage`、其直接公共依赖、宿主 API/Java DTO 或目标只读实测证明的组件加载候选；按当前模板惰性读取并保持账号权限隔离。无候选、权限不足、读取失败/超时与未知规则必须用去重的字段级中文原因区分，绝不伪造外部对象。
3. 当前完整选中路径继续作为生成、换一组、保存复验和批量准备的共同约束；关键条件字段由服务端锁定，换一组不得改变路径验证结果。
4. 表单生成、部分结果、阻断、模板更新和保存提示改为可关闭的 Naive UI 悬浮通知，不占用表单 iframe 布局；超时、异常、导航和卸载均清理 loading 与旧通知。
5. 根目录回归覆盖身份 ID/名称与 iframe 回显、真实候选/无候选去重原因、完整路径锁定、浮层关闭和异步清理；完成 F-010、Go/API/MySQL、双前端 typecheck/build、现有 19080 只读 API 实测后，才重新进入 `ready_for_manual`。
