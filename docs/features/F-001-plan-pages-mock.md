...
- [ ] `/plans` 与 `/plans/new` 路由可用，列表和新建页均处于已验收应用壳内。
- [ ] 名称、状态筛选及清空真实作用于本地 mock 数据。
- [ ] 四种状态各映射且只映射一个主要操作，点击后只显示静态边界提示。
- [ ] 新建表单联动、并行条件字段和校验边界符合范围；成功后不保存、不跳转路径页。
- [ ] 1366×768 桌面视口下表格可在内容区水平处理，不撑出 body。
- [ ] 当前范围测试、前端类型检查和生产构建全部通过。
- [ ] 状态更新为 `ready_for_manual` 并停止，不开始 F-002。
