...
- 验证后按来源分别显示流程模板、已发流程和待发流程选择区；三类选择区均支持本地搜索、固定高度虚拟滚动、分批追加、空结果、加载中和到底状态。三个来源与未验证空态共用稳定外壳，仅在外壳内进行短促淡入和轻微位移过渡；减少动态效果时取消该过渡。
...
- 表单在内容区居中，返回入口稳定吸顶在内容区左上方；吸顶层保留固定占位、同一主题表面和静态定位，不使用位移动画。上方配置与流程选择区之间使用单一语义分隔线和合理留白。
- 验证账号成功或切换来源后，仅当搜索区未完整可见时才最小距离平滑滚动到搜索区并聚焦；选中候选后优先引导到未填写的启动时间、无效并发数或提交操作。减少动态效果时滚动不使用平滑动画。
...
- [x] 定时启动、并行条件字段、居中布局、稳定吸顶返回入口和提交校验符合本轮范围。
- [x] 流程选择区几何稳定、分隔边界清晰，且仅在必要时以可预测方式引导下一步。
...
- `test/unit/frontend/plan_form_test.mjs`：来源可用性、三类 mock 语义、搜索过滤、固定批次追加、下一步优先级和最小滚动距离。
- `test/integration/plan_pages_structure.sh`：路由、账号验证状态、稳定选择外壳、`NDivider`、`NVirtualList`、减少动态效果、渐进引导、定时启动、动态 `FormRules` 和无请求边界。
...
4. 进入 `/plans/new`，确认表单居中且“返回测试计划”稳定吸顶在内容区左上方；滚动时不改变高度、不位移弹动。
...
7. 在三类列表中分别核对搜索、整行选择、选中态、固定高度滚动、滚动到底追加、空结果和到底提示；切换来源时旧选择被清空，选择区外壳高度不突变。
...
11. 验证账号、切换来源与选中候选时，确认只有目标不完整可见才出现最小距离引导；减少动态效果时不出现淡入位移或平滑滚动。
12. 在浅色与深色下核对列表、分隔线、主题工具栏和侧栏收缩；确认 body 不滚动，内容区独立滚动。
...
- 2026-07-27：普通流式返回入口、稳定流程选择外壳、语义分隔线和渐进下一步引导完成，当前范围自动验证通过；重新进入 `ready_for_manual`，不开始 F-002。
- 2026-07-27：用户调整人工反馈，确认返回入口仍需要稳定吸顶；从 `ready_for_manual` 返回 `implementing`。
- 2026-07-27：稳定吸顶返回入口和当前范围自动验证完成，重新进入 `ready_for_manual`；不开始 F-002。
- 2026-07-27：用户明确回复“开始下一个任务”，确认 F-001 人工验收通过；状态进入 `accepted`。
