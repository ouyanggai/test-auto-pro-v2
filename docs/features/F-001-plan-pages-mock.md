...
- 并行最大并发数只在并行模式显示并校验 2 至 20；切换运行方式会清理已隐藏字段的旧校验。
- 定时时间以“定时启动”开关控制，关闭后不渲染时间选择器并清空时间值，开启后要求选择启动时间。
...
- [x] 账号静态验证、来源禁用与失效联动可操作，且没有真实登录或请求。
- [x] 三类来源分别显示可搜索虚拟增量列表，切换时没有不兼容选择或旧结果。
- [x] 定时启动、并行条件字段、居中布局、粘附返回入口和提交校验符合本轮范围。
...
- `test/unit/frontend/plan_form_test.mjs`：来源可用性、三类 mock 语义、搜索过滤和固定批次追加边界。
- `test/integration/plan_pages_structure.sh`：路由、账号验证状态、三类独立选择区、`NVirtualList`、定时启动、动态 `FormRules` 和无请求边界。
...
4. 进入 `/plans/new`，确认表单居中且“返回测试计划”滚动时粘附在内容区左上方。
5. 空账号点击“验证账号”或提交，确认错误只出现在真实账号字段下；输入任意非空账号后确认只进入“本地静态验证”状态，不声称登录平台或生成 SID。
6. 未验证时确认“已发”“待发”不可选；验证后确认三个来源都可用，且分别显示“流程模板”“已发流程”“待发流程”，页面不出现统一“目标流程”。
7. 在三类列表中分别核对搜索、整行选择、选中态、固定高度滚动、滚动到底追加、空结果和到底提示；切换来源时旧选择被清空。
8. 编辑已验证账号，确认验证立即失效、当前来源回到“新发起”、旧选择清空；重新验证后加载新一轮本地 mock。
9. 开启“定时启动”后才出现启动时间，关闭后时间被清空；串并切换时并发字段及旧校验正确清理。
10. 未验证账号或未选择当前来源对象时提交应失败；填写完整后只短暂显示静态成功提示，不保存、不进入路径页。
11. 在浅色与深色下核对列表、分隔线、主题工具栏和侧栏收缩；确认 body 不滚动，内容区独立滚动。
...
- 2026-07-27：账号静态验证、三类虚拟增量选择列表、定时启动及当前范围自动验证完成，重新进入 `ready_for_manual`；不开始 F-002。
- 2026-07-27：用户反馈返回入口吸顶弹动、动态候选区页面跳动、配置区与候选区粘连，要求补充渐进且可预测的下一步引导；从 `ready_for_manual` 返回 `implementing`。
