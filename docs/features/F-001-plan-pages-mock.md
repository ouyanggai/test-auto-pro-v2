...
- 验证后按来源分别显示流程模板、已发流程和待发流程选择区；三类选择区均支持本地搜索、固定高度虚拟滚动、分批追加、空结果、加载中和到底状态。三个来源与未验证空态共用稳定外壳，仅在外壳内进行短促、自然的淡入和轻微位移过渡。
...
- 验证账号成功或切换来源后，仅当搜索区未完整可见时才最小距离平滑滚动到搜索区并聚焦；选中候选后优先引导到未填写的启动时间、无效并发数或提交操作，滚动与内容切换保持自然连贯。
...
- `test/integration/plan_pages_structure.sh`：路由、账号验证状态、稳定选择外壳、`NDivider`、`NVirtualList`、自然过渡、渐进引导、定时启动、动态 `FormRules` 和无请求边界。
...
11. 验证账号、切换来源与选中候选时，确认只有目标不完整可见才出现最小距离引导，淡入位移与平滑滚动自然连贯且不会反复抢夺视口。
