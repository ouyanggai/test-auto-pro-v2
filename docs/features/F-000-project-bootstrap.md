...
3. 核对顶栏横跨全宽、侧栏从顶栏下方开始、两者有 1px 分隔线；浏览器本身不滚动，只有内容超出时主内容区滚动。
4. 点击左上角“深色/浅色”入口，核对顶栏、侧栏、内容、文字、边线和选中态均随主题切换；刷新后确认主题保持。
5. 核对产品名与“测试计划、运行记录、系统设置”三个中文入口，并依次点击确认路由和占位内容。
6. 打开 `http://127.0.0.1:19080/api/health`，核对固定 JSON。
7. 修改任意项目 Go 源码并保存，核对后端终端显示重编译和重新运行。
8. 在两个终端分别按 `Ctrl+C` 停止前端和后端。
