...
- 节点保存请求只接收当前选中节点的不透明键、合法人员选择策略和版本化有序动作计划；表单保存使用独立请求接收表单运行时校验后的模型。人员策略支持目标真实默认、合法范围内手动选择、带稳定 seed 的确定性随机，以及模板允许时的全选；随机或手动候选不得越过目标模板范围。服务端每次重新解析当前目标元数据，验证节点、人员、动作和参数仍属于当前路径，保存不执行目标平台写操作。
- 公开动作计划使用语义化结构：发起节点只有提交；审批/协同节点由零到多个独立加签节点和恰好一个处理结果组成。处理结果默认同意，也可替换为当前目录允许的不同意、暂存、回退上一级或移交；前端和公开 DTO 不暴露内部兼容分组。动作目录只公开 `enabled`、`disabledReason` 和必要参数约束。回退分析反向穿过路由节点，只接受唯一直接上一审批节点：直接上级为发起人时禁用，多个分支前驱或无法静态确定时也禁用并说明运行时任务链复验。加签依据目标 `AddCounterSign` 新增审批节点，每个加签节点保存独立的处理人候选与策略，不以当前节点主处理人是否可编辑为开关；移交使用当前节点受限候选，按目标多选规则至少选择一人、至多当前合法候选数，并结束当前任务处理。取回、撤销、废弃、催办、跟踪、删除和转发不属于 F-007 节点配置。
- `test_execution_path_configs` 继续保持每条路径一行，复用现有 JSON 字段保存版本化人员策略和动作计划，不新增迁移。新保存会确定性编码为单个内部处理分组，其中是零到多个 `add_sign`，最后恰好一个终止处理结果；发起节点只编码提交。配置模式版本、已明确确认节点集合、完整表单数据、表单状态、校验结果、稳定 seed、生成字段路径、人工覆盖字段路径和样本来源摘要继续保留；字段/动作仍按节点隔离。节点保存采用事务内合并而不是整份替换，表单保存只更新表单区。旧记录只有在能无损还原为该语义结构时才投影；重复提交或同意、多个处理结果、非加签重复、结果后追加或其他歧义结构均标记 `affected`，必须重新确认。
...
- `test_execution_path_configs` 保持每路径一行，不新增数据库迁移。现有动作 JSON 在读取时仅作一次性迁移输入，事务内写回版本化的节点动作和路径循环结构；公开 API 与前端草稿不再混入 `arrivals`、`steps`、`addSignNodes`、`result` 或 `combinationCount`。无法无损转换的旧值不自动形成循环，按待配置投影。
- 后续真实执行的后端大纲固定为：每次目标动作前读取实例、待办、权限、批次和节点快照；只按当前计划中的一个状态迁移调用一个目标接口；结果不确定时先查询实例与待办再决定重试；不同意后等待实例 `rejected` 再执行重提；回退后以新批次和新待办为事实继续。该执行编排不属于 F-008，F-008 不实现目标写入、调度或运行记录。实现必须在 `test/` 按现有后端、前端与契约分类覆盖动作次数、两种回路、非法跨支组合、前驱回退限制、批量预设、旧值迁移和幂等对账，并只运行当前功能相关验证。
