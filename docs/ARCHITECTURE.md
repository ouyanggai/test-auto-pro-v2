...
- 前端在同一现有画布上区分普通浏览、已保存路径只读详情和页面全屏编辑，不复制主图节点或引入第二套图引擎。普通画布仅由画布提供带文字的页面全屏按钮；页面全屏后才渲染页面注入的已保存列表与符合来源及数量限制的新增入口，同时全屏退出仍使用文字按钮。只有编辑、新建和复制开放条件/手动分支标签。条件/手动分支标签至少 32px 高，并行分支显示“并行必经”且不可取消。
