...
- F-007 的 `configured` 只表示工具侧当前路径的必需节点已逐一确认；表单是否满足由独立 `dataStatus` 表示。两者都不改变 `test_plans.status` 为可运行，不生成运行记录，也不调用目标平台写接口。附件上传、高级 AI 语义生成、跨模板学习、成功断言和运行时动作进入后续切片。
