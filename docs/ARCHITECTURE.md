...
- 动作弹窗草稿只维护当前节点的动作数组；循环弹窗草稿只维护起点、终点、返回方式和循环次数。前端根据服务端返回的路径顺序自动派生只读成员摘要，不能把成员顺序、跨支连接或旧兼容字段写回请求。每个弹窗独立处理未保存关闭确认、聚焦错误项和幂等保存，避免页面级草稿相互污染。
- 公开 DTO 拆为节点 `actionConfiguration` 与路径 `actionCycles`。节点动作项只含不透明键、动作类型、次数、可选回退目标和受限人员策略；循环只含稳定键、起点/终点节点键、按路径顺序的成员节点键、循环次数和返回方式。浏览器不得提交目标内部 ID、任意节点顺序、模拟到达编号或可执行接口参数。
...
- `internal/analyzer` 以已保存路径的拓扑顺序构建不可伪造的循环候选：成员必须连续，且结束动作后的真实状态只能通过“不同意 -> 发起人 -> 提交 -> 起点”或“回退 -> 精确起点”闭合。它不使用名称、显示顺序或旧 JSON 猜测回退目标；任何节点、分支、人员或权限无法静态证明时只返回不可配置原因。
...
- 运行编排在后续切片消费该状态机定义；F-008 不调用任何目标写接口。实现必须在 `test/` 按现有后端、前端与契约分类覆盖动作次数、两种回路、非法跨支组合、回退目标不精确、旧值迁移和幂等对账，并只运行当前功能相关验证。
