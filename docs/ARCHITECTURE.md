...
- 前端在现有画布增加“浏览 / 页面全屏浏览 / 页面全屏线路选择”三态，不复制主图节点或引入第二套图引擎。只有线路选择态开放条件/手动分支标签；点击“线路选择”先进入页面内 `fixed` 全屏，再开启编辑。条件/手动分支标签至少 32px 高，并行分支显示“并行必经”且不可取消。
- 线路选择态使用覆盖在画布右侧的轻量 DOM 路径摘要面板，按当前分析结果实时投影节点、已选分支、并行组和下一待选点；该摘要不建立第二份业务模型、不落库，内部独立滚动并可收起。路径管理、剩余数量、保存和删除集中在面板内，避免全宽工具栏遮挡画布。
...
- 路径草稿只保存在当前页面内，完整后由明确的“保存路径”操作提交；保存中的重复点击被禁用，失败保留草稿。退出线路选择或页面全屏不丢草稿，再次进入显示“继续选择”；保存路径默认名包含序号并可就地改名，列表始终单独显示稳定序号。新发起可新增、复制或一键生成全部路径，已发/待发已有一条后不再显示这些入口，删除需要确认。
