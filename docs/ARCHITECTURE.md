...
- 画布边状态显式区分 `selected`、`candidate`、`dimmed`：只有已选和自动必经线路运行动画并使用高对比主题主色与更粗线宽；待选分支静止但保持明确点击态；未选或未参与线路静止弱化，标签仍可读和可操作。`prefers-reduced-motion: reduce` 下全部停止动画，状态仍由线宽、颜色与文字表达。
