...
- 动作计划在 UI 中以平铺动作行组织，每行直接设置动作、执行次数和必要参数；内部 `arrivals` 仅作为持久化兼容结构，由单一纯函数与动作行确定性互转。发起节点支持提交，审批/协同节点支持同意、不同意、暂存、回退上一级、加签和移交。回退分析反向穿过路由节点，只接受唯一直接上一审批节点：直接上级为发起人时禁用，多个分支前驱或无法静态确定时也禁用并说明运行时任务链复验。加签、移交使用各自独立且受目标模板候选约束的人员策略，不能与主处理人是否可编辑绑定；移交按目标多选规则至少选择一人、至多当前合法候选数，并结束当前任务处理。结束当前任务的动作最多 10 次、每条路径动作总数最多 100，禁止任意脚本和无界循环。取回、撤销、废弃、催办、跟踪、删除和转发不属于 F-007 节点配置。
