...
- 新增 `test_execution_paths` 保存计划、稳定路径序号、幂等创建键和时间；新增 `test_execution_path_choices` 保存每条路径在真实条件/手动路由上的分支选择，主键保证同一路由只有一个选择。并行分支、派生节点集合和整张流程图不落库。
...
- `GET /api/plans/{id}/execution-paths` 读取保存路径；`POST /api/plans/{id}/execution-paths` 创建完整路径；`PUT /api/plans/{id}/execution-paths/{pathId}` 替换该路径选择；`DELETE /api/plans/{id}/execution-paths/{pathId}` 删除路径。写接口只允许尚未产生运行事实的待配置计划，路径和计划不匹配时返回稳定错误。
...
- 画布边状态显式区分 `selected`、`candidate`、`dimmed`：只有已选和自动必经线路运行动画并使用高对比主题主色与更粗线宽；当前可达路由的待选及同路由备选分支静止但保持明确点击态；尚未到达的后续分支静止弱化并保留名称，暂不允许越级选择。`prefers-reduced-motion: reduce` 下全部停止动画，状态仍由线宽、颜色与文字表达。
- 进入选择态以及每次分支选择后，前端按确定性遍历顺序取得下一待选路由；若它不在扣除悬浮面板后的安全视口内，只平移最小距离且不改变缩放，随后在节点附近显示短暂、可关闭且支持 `aria-live` 的操作提示。路径完整时改为提示保存，不自动提交。
- 路径草稿只保存在当前页面内，完整后由明确的“保存路径”操作提交；保存中的重复点击被禁用，失败保留草稿。退出线路选择或页面全屏不丢草稿，再次进入显示“继续选择”；保存路径以稳定序号显示，新发起可新增或复制，已发/待发已有一条后不再显示新增入口，删除需要确认。
