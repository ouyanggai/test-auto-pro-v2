...
- 动作计划在 UI 中以“动作次数”和“第 N 次”组织，内部 `arrivals` 仅作为持久化兼容名，序号连续且有界；发起节点支持提交，审批/协同节点支持同意、不同意、暂存、回退上一级、加签和移交。回退目标只能是当前路径上更早且可达的业务节点；加签、移交使用各自独立但同样受目标模板候选约束的人员策略，不能与主处理人是否可编辑绑定。移交按目标多选树规则至少选择一人、至多当前合法候选数，仍只能作为本次动作最后一步。每节点最多 10 次、每条路径最多 100 个动作步骤，禁止任意脚本和无界循环。取回、撤销、废弃、催办、跟踪、删除和转发不属于 F-007 节点配置。
- `test_execution_path_configs` 继续保持每条路径一行，复用现有 JSON 字段保存版本化人员策略和动作计划，除非实现证明确实无法兼容，否则不新增迁移。配置模式版本、已明确确认节点集合、完整表单数据、表单状态、校验结果、稳定 seed、生成字段路径、人工覆盖字段路径和样本来源摘要继续保留；字段/动作仍按节点隔离。节点保存采用事务内合并而不是整份替换，表单保存只更新表单区。旧版仅有同意/不同意或仅凭一行即为 configured 的记录应迁移为首次到达计划；无法准确映射时保留原值并标记 `affected`，必须重新确认。
