...
- 动作计划在 UI 中以平铺动作行组织，每行直接设置动作、执行次数和必要参数；内部 `arrivals` 仅作为持久化兼容结构，由单一纯函数与动作行确定性互转。动作目录公开 `enabled`、`disabledReason` 和单动作 `maxCount`，保存校验与存量投影消费同一权威限制：提交和同意均固定 `maxCount=1`，恶意或存量重复值分别被拒绝或标记 `affected`；审批/协同默认生成一条同意，其他终止处理结果替代该默认值，不能与同意并存。其余允许重复的动作继续受单节点 10 次和全路径 100 步约束。前端按目录即时归一并防止重复，动作说明和禁用原因只在信息弹层集中展示。动作次数表示真实执行次数而非网络重试，每次执行仍需重新绑定真实实例与任务。审批/协同节点支持同意、不同意、暂存、回退上一级、加签和移交。回退分析反向穿过路由节点，只接受唯一直接上一审批节点：直接上级为发起人时禁用，多个分支前驱或无法静态确定时也禁用并说明运行时任务链复验。加签依据目标 `AddCounterSign` 新增审批节点，使用独立的加签节点处理人候选与策略，不以当前节点主处理人是否可编辑为开关；移交使用当前节点受限候选，按目标多选规则至少选择一人、至多当前合法候选数，并结束当前任务处理。取回、撤销、废弃、催办、跟踪、删除和转发不属于 F-007 节点配置。
- `test_execution_path_configs` 继续保持每条路径一行，复用现有 JSON 字段保存版本化人员策略和动作计划，除非实现证明确实无法兼容，否则不新增迁移。配置模式版本、已明确确认节点集合、完整表单数据、表单状态、校验结果、稳定 seed、生成字段路径、人工覆盖字段路径和样本来源摘要继续保留；字段/动作仍按节点隔离。节点保存采用事务内合并而不是整份替换，表单保存只更新表单区。旧版仅有同意/不同意的记录可准确映射时转成动作列表，无法准确映射时保留原值并标记 `affected`，必须重新确认。
