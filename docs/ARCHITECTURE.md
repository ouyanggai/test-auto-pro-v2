...
- 动作计划按节点的第几次到达组织，访问序号连续且有界；发起节点支持提交，审批/协同节点支持同意、不同意、暂存、回退上一级、加签和移交。回退目标只能是当前路径上更早且可达的业务节点；加签和移交复用人员策略。每节点最多 10 次到达、每条路径最多 100 个动作步骤，禁止任意脚本和无界循环。取回、撤销、废弃、催办、跟踪、删除和转发不属于 F-007 节点配置。
