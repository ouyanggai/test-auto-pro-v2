...
- 新增 `test_execution_paths` 保存计划、稳定路径序号、幂等创建键和时间；新增 `test_execution_path_choices` 保存每条路径在真实条件/手动路由上的分支选择，主键保证同一路由只有一个选择。创建路径时先锁定所属计划行，再分配当前历史最大序号加一，避免并发创建得到相同序号。并行分支、派生节点集合和整张流程图不落库。
- 两张表使用独立向前迁移；路径删除级联删除自己的选择，不删除计划或目标平台数据。新路径序号取当前计划历史最大值加一，删除后不重排，保证后续配置和运行记录引用稳定。
