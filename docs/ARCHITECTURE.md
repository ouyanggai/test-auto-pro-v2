...
- 动作计划在 UI 中以“动作次数”和“第 N 次”组织，内部 `arrivals` 仅作为持久化兼容名，序号连续且有界；发起节点支持提交，审批/协同节点支持同意、不同意、暂存、回退上一级、加签和移交。回退目标只能是当前路径上更早且可达的业务节点；加签、移交使用各自独立但同样受目标模板候选约束的人员策略，不能与主处理人是否可编辑绑定。每节点最多 10 次、每条路径最多 100 个动作步骤，禁止任意脚本和无界循环。取回、撤销、废弃、催办、跟踪、删除和转发不属于 F-007 节点配置。
