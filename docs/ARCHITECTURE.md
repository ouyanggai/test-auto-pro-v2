...
- 前端在现有画布增加路径选择状态，不复制节点。条件/手动分支使用至少 32px 高的可点击标签；并行分支显示“并行必经”且不可取消。选中线路使用主题主色，未选候选保持可读但弱化；页面全屏、拖动、缩放、流动线条和减少动态效果继续有效。
- 路径草稿只保存在当前页面内，完整后由明确的“保存路径”操作提交；保存中的重复点击被禁用，失败保留草稿。保存路径以稳定序号显示；新发起可新增或复制，已发/待发已有一条后不再显示新增入口。删除需要确认。
