...
- 应用壳使用 Naive UI 文档站式的扁平布局：顶栏横跨全宽，侧栏从顶栏下方开始，主内容占用剩余空间；使用组件库语义色和 1px 分隔线，不使用卡片套层、渐变、阴影或装饰图形。
- 浏览器根节点固定为 `100dvh` 且不滚动；顶栏、侧栏固定，主内容区独立 `overflow-y: auto`，侧栏仅在菜单超出时自身滚动。Flex/Grid 子项使用最小尺寸约束，避免撑出 viewport。
