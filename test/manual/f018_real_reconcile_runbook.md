...
## 服务重启后怎么继续（2026-09-05 根治后）

对账现场不再依赖进程内存：停在待对账的路径运行在服务重启后，只要再调一次对账接口
（或直接打开运行详情页，进入待对账会自动对账一次），现场就会按已落库的运行事实重建，
随后三个恢复动作照常可用。重建过程只读（会登录目标并重读本步事实），不发任何写请求。

实测（运行 12）：重启后详情页的对账区域一度为空，`POST /api/runs/12/reconcile` 用 10.8 秒
（主要是目标登录）完成重建并给出真实五维读数。该运行的尝试行早于迁移 029，没有写前基准，
因此三个实例维度如实报「写之前的目标事实基准没有落库，无从比较」，动作痕迹维度真实读到了本步节点的发起痕迹，
结论仍无法判定——这是诚实结果，不是缺陷。迁移 029 之后落账的尝试都带基准，重建后能得到与重启前一致的结论。
