...
grep -Fq 'templateGroupName' "${candidate_file}"
grep -Fq 'type="info"' "${candidate_file}"
grep -Fq 'candidate.groupName' "${project_root}/web/src/features/plans/presentation.ts"
grep -Fq 'submittedStatusName' "${api_file}"
...
grep -Fq 'statusName: item.statusName || submittedStatusName(item.status)' "${api_file}"
...
! grep -RInE 'templateCompanyName|companyName: item\.companyName|getParentCompanyList|formTemplateBizRelevanceVoList' "${project_root}/web/src/features/plans" "${project_root}/internal/adapter/target" >/dev/null
