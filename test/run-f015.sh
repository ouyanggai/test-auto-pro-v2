#!/usr/bin/env bash

set -euo pipefail

project_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "${project_root}"

# 集成用例要连真实 MySQL 与真实目标，配置只放本机被 Git 忽略的 .env.local。
export TEST_AUTO_PRO_PLAN_DB_ENV_FILE="${TEST_AUTO_PRO_PLAN_DB_ENV_FILE:-${project_root}/.env.local}"
export TEST_AUTO_PRO_TARGET_ENV_FILE="${TEST_AUTO_PRO_TARGET_ENV_FILE:-${project_root}/.env.local}"
if [ -f "${project_root}/.env.local" ]; then
  set -a
  # shellcheck disable=SC1091
  . "${project_root}/.env.local"
  set +a
fi

printf '%s\n' '[F-015] 编译与静态检查'
go build ./...
go vet ./internal/service/... ./internal/api/... ./test/unit/backend/run_readiness ./test/integration

printf '%s\n' '[F-015] 运行前检查单元测试（含竞态检测）'
go test -race -count=1 ./test/unit/backend/run_readiness/...

printf '%s\n' '[F-015] 真实 MySQL 集成测试'
integration_log="$(mktemp -t f015-integration)"
trap 'rm -f "${integration_log}"' EXIT
if ! go test -count=1 -v -run 'TestF015Readiness' ./test/integration 2>&1 | tee "${integration_log}"; then
  exit 1
fi
if grep -Eq -- '^[[:space:]]*--- SKIP' "${integration_log}"; then
  printf '%s\n' '[F-015] 集成测试存在跳过用例，判定为失败' >&2
  exit 1
fi
for required in TestF015ReadinessReadsRealDatabaseFacts TestF015ReadinessStorageFailureIsNotReportedAsMissingPlan TestF015ReadinessMissingPlanStillReportsNotFound; do
  if ! grep -Eq -- "^[[:space:]]*--- PASS: ${required}" "${integration_log}"; then
    printf '%s\n' "[F-015] 缺少必需的集成用例通过记录：${required}" >&2
    exit 1
  fi
done

printf '%s\n' '[F-015] 写端点白名单为空'
./test/contracts/f015/target_write_whitelist.sh

printf '%s\n' '[F-015] 接口与前端结构契约'
./test/contracts/f015/api_and_frontend_structure.sh

printf '%s\n' '[F-015] 前端构建'
(cd web && npm run build >/dev/null)

printf '%s\n' '[F-015] 日志目录未进入版本库'
if ! git check-ignore -q logs/app.log; then
  printf '%s\n' '[F-015] logs/ 必须被 .gitignore 忽略' >&2
  exit 1
fi
git status --porcelain logs 2>/dev/null | grep -q . && {
  printf '%s\n' '[F-015] logs/ 出现在待提交列表' >&2
  exit 1
}

git diff --check
printf '%s\n' 'F-015 定向验证完成'
