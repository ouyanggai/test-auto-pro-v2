...
test('T05 数据工作区按原始 values 协议连接复制 runtime', () => {
  ...
  assert.match(frame, /changedFields: props\.form\.branchPatches\.map/)
  assert.match(frame, /props\.form\.viewKey/)
  assert.doesNotMatch(frame, /props\.form\.viewName/)
  assert.match(runtime, /this\.values = clonePlain\(payload\.values \|\| \{\}\)/)
  assert.match(runtime, /replayFieldChangeEvents/)
...
test('T05 表单提示改为右侧可收起悬浮面板并给出决定路径的关键字段', () => {
  ...
  assert.match(panel, /function fieldLabel\(field: PathConfigKeyField\)/)
  assert.doesNotMatch(panel, /<code>\{\{ field\.path \}\}<\/code>/)
...
test('T05 保存换路需要确认令牌且取消不触发写入', () => {
  ...
  // 更换历史数据只是打开基础表单数据弹窗，真实分支补丁由服务端在读取数据工作区时完成。
  assert.match(view, /更换历史数据/)
