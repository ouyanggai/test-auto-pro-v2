...
if [[ -f "${project_root}/.env.local" ]]; then
  TEST_AUTO_PRO_PLAN_DB_ENV_FILE="${project_root}/.env.local" go test -count=1 ./test/integration -run '^TestF009PathPreparationMySQLCheckpoints$'
elif [[ -n "${PLAN_DB_HOST:-}" ]] && [[ -n "${PLAN_DB_USER:-}" ]] && [[ -n "${PLAN_DB_PASSWORD:-}" ]]; then
  go test -count=1 ./test/integration -run '^TestF009PathPreparationMySQLCheckpoints$'
else
