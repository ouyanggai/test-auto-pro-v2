<template>
  <div>
    <el-button type="primary" :disabled="disabled" @click="handleOpenDialog">{{ placeholder || '库存材料选择' }}</el-button>
    <el-dialog
      ...
      this.$axios.post(Api.inventoryManage.getSetLedgerGoods,
        ...
        res => {
          ...
          if (res.isSuccess) {
            console.log(res, res);
            if (res.data && res.data.length) {
              this.tableData = res.data.filter(item => (item.totalCount && item.totalCount > 0));
