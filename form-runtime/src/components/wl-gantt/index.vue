<template>
  <div class="wl-gantt" id="wl-gantt">
    <!-- 甘特图区 -->
    <el-table ref="wl-gantt" class="wl-gantt-table" :fit="fit" :size="size" :load="load" :lazy="lazy" :border="border"
      :data="selfData" :stripe="stripe" :row-key="rowKey" :row-style="rowStyle" :class="dateTypeClass"
      :cell-style="cellStyle" :height="height" :max-height="maxHeight" :tree-props="selfProps" :current-row-key="rowKey"
      :row-class-name="rowClassName" :cell-class-name="cellClassName" :expand-row-keys="expandRowKeys"
      :header-row-style="headerRowStyle" :header-cell-style="headerCellStyle" :default-expand-all="defaultExpandAll"
      :header-row-class-name="headerRowClassName" :highlight-current-row="highlightCurrentRow"
      :header-cell-class-name="headerCellClassName" @header-contextmenu="handleHeaderContextMenu"
      ...
      @cell-mouse-leave="handleMouseLeave" @expand-change="handleExpandChange" @filter-change="handleFilterChange"
      @cell-dblclick="handleCellDbClick" @header-click="handleHeaderClick" @row-dblclick="handleRowDbClick"
      @sort-change="handleSortChange" @cell-click="handleCellClick" @select-all="handleSelectAll"
      @row-click="handleRowClick" @select="handleSelect">
      <template v-if="!ganttOnly">
        ...
        <el-table-column label="名称" min-width="360" class-name="name-col" fixed :prop="selfProps.name"
          :formatter="nameFormatter" :show-overflow-tooltip="name_show_tooltip">
        </el-table-column>
      </template>
      <!-- year and mouth gantt  一年中按月-->
      <template v-if="self_date_type === 'yearAndMonth'">
        <el-table-column :resizable="false" v-for="year in ganttTitleDate" :label="year.name" :key="year.id">
          <el-table-column class-name="wl-gantt-item" v-for="month in year.children" :resizable="false" :key="month.id"
            :label="month.name">
            <template slot-scope="scope">
      ...
      <template v-else-if="self_date_type === 'yearAndWeek'">
        <el-table-column :resizable="false" v-for="i in ganttTitleDate" :label="i.full_date" :key="i.id">
          <el-table-column class-name="wl-gantt-item" v-for="t in i.children" :resizable="false" :key="t.id"
            :label="t.name">
            <template slot-scope="scope">
      ...
      <!-- mouth and day gantt 一月中按天-->
      <template v-else>
        <el-table-column :resizable="false" v-for="i in ganttTitleDate" :label="i.full_date" :key="i.id">
          <el-table-column class-name="wl-gantt-item" v-for="t in i.children" :resizable="false" :key="t.id"
            :label="t.name" :width="25">
            <template slot-scope="scope">
    ...
    <div v-show="infoCard.show" class="wl-info-card" :style="infoCardStyle">
      <slot name="info-card" :row="infoCard.row" :column="infoCard.column" :cell="infoCard.cell"
        :event="infoCard.event"></slot>
    </div>
  </div>
</template>
...
// import ContextMenu from './components/wl-contextmenu'; // 导入日期js
...
export default {
  ...
  computed: {
    ...
    ganttTitleDate() {
      // 分解开始和结束日期
      const start_date_spilt = dayjs(this.self_start_date)
        .format('YYYY-M-D')
        .split('-');
      const end_date_spilt = dayjs(this.self_end_date)
        .format('YYYY-M-D')
        .split('-');
    ...
    selfProps() {
      return {
        ...
        id: 'id', // id字段
        pid: 'pid', // pid字段
        startDate: 'startTime', // 开始时间字段
        realStartDate: 'realStime', // 实际开始时间字段
        endDate: 'endTime', // 结束时间字段
        realEndDate: 'realEtime', // 实际结束时间字段
  ...
  methods: {
    ...
    startDateChange(row) {
      // 如果将开始时间后移，结束时间也应后移
      const _delay = this.timeIsBefore(
        row._oldStartDate,
      ...
      // 如果开始早于项目开始，则把项目开始提前
      const _early_project_start = this.timeIsBefore(
        row[this.selfProps.startDate],
        this.self_start_date
      );
    ...
    endDateChange(row) {
      // 如果开始晚于项目结束，则把项目结束延后
      const _early_project_end = this.timeIsBefore(
        this.self_end_date,
        row[this.selfProps.endDate]
      );
    ...
    preFormat(row) {
      ...
      if (Array.isArray(data)) {
        ...
        let _pre_text = '';
        data.forEach(i => {
          const _act = this.self_data_list.find(
    ...
    mouthAndDayTitleDate(start_year, start_mouth, end_year, end_mouth) {
      ...
      let middle_mouths = [];
      for (let i = 1;i < year_diff;i++) {
        const endIsLeap = this.isLeap(start_year + i);
        const year_mouths = this.generationMonths(
          start_year + i,
          1,
          13,
          endIsLeap
        );
        middle_mouths = [...middle_mouths, ...year_mouths];
      }
      // 处理结束月份
      const endIsLeap = this.isLeap(end_year);
      const end_mouths = this.generationMonths(
      ...
      console.log(start_mouths, middle_mouths, end_mouths);
      return start_mouths.concat(middle_mouths).concat(end_mouths);
    },
    /**
     * 生成月份函数
     * year: Number 当前年份
    ...
    parentStartDateToChild(item) {
      ...
      if (_child_early_parent) {
        ...
        // 修正子节点结束时间
        const _to_endDate = this.timeAdd(
          item[this.selfProps.startDate],
          item._cycle
        );
    ...
    checkPreTaskValidity(item) {
      ...
      if (this.preMultiple) {
        ...
        for (const i of _pre_exist) {
          ...
          const _pre_par_chi = this.targetInParentsOrChildren(
            item,
            _pre_target
          );
...
.wl-gantt {
  $ganttColor: 1px solid #016fbd;

  .wl-item-on {
    @include normalStyle;
    background: linear-gradient(180deg, #bedfff 0%, #329aff 100%);
  }

  .wl-real-on {
    @include normalStyle;
    top: 81.3%;
    background: linear-gradient(180deg, #c3f0c3 0%, #43d144 100%);
  }

  .wl-item-start {
    border-left: $ganttColor;
  }

  .wl-item-end {
    border-right: $ganttColor;
  }
