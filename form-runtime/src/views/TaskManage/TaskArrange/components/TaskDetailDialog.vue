<!--
 * @Descripttion: 审核任务详情(这个页面好像是没用了)
 * @Author: zhengzetao
 * @Date: 2022-4-15
-->
<template>
  <component v-bind:is="currentComponent" :visible="visible" :title="arrangeType == 'check' ? '查看任务' : '审核任务'" width="80%"
    :close-on-click-modal="false" @close='handleClose'>

    <div class="task-container">
      <div :class="arrangeType == 'check' ? 'append' : 'lower'">
        <div style="padding: 10px;background: #fff;">
          ...
          <div class="info-wrap">
            <div class="info-item">
              <div class="info-item-title">优先级</div>
              <div>
                <el-tag :type="pointTabObj[basicInfoData.pointTab].type">{{ pointTabObj[basicInfoData.pointTab].value }}</el-tag>
              </div>
            </div>
          </div>
            ...
            <div v-if="planBusinessList.length > 0">
              <div v-for="planBusiness in planBusinessList" :key="planBusiness.businessId" style="margin-bottom: 5px;">
                ...
                <el-button v-if="planBusiness.previewUrl" size="mini" type="text"
                  ...
                  <!-- @click="previewBusinessFile(planBusiness.previewUrl) -->
                  预览
                </el-button>
              </div>
            </div>
              ...
              <div v-if="Object.keys(basicInfoData).length && basicInfoData.executeLogList.length">
                <div v-for="(item, index) in basicInfoData.executeLogList" :key="index">
                  <div v-if="item.executePhase == 'has_been_sent_withdraw'"
                    ...
                    <div>
                      {{ item.user && item.user.name + ' 于 ' + item.createDate + ' 终止此项工作' }}
                    </div>
                    <div v-html="item.executeExplain">
                    </div>
                  </div>

                  <div v-if="item.executePhase == 'submit'" style="padding: 12px 10px;border-bottom: 12px solid #f2f2f2;">
                    <div>
                      {{ item.user && item.user.name + ' 于 ' + item.createDate + ' 提交审核' }}
                    </div>
                    <div v-html="item.executeExplain">
                    </div>
                  ...
                  <div v-if="item.executePhase == 'pass' || item.executePhase == 'not_pass'"
                    ...
                    <div>
                      {{
                        item.examiner && item.examiner.name + ' 于 ' + item.createDate + (item.executePhase == 'pass' ? ' 审核通过' : '审核不通过')}}
                    </div>
                    <div v-html="item.executeExplain">
                    </div>
                  </div>
    ...
    <div class="check-wrap" v-if="arrangeType != 'check'">
      <el-form :model="auditForm" ref="auditForm" label-width="100px" label-position="top">
        <!-- <el-form-item
            prop="opinion"
            label="审核意见:"
          >
            <el-select v-model="auditForm.opinion">
              <el-option
                label="通过"
                value="1"
                key="1"
          ...
          <el-form-item prop="content" label="附言:">
            <el-input type="textarea" :rows="6" maxlength="500" show-word-limit v-model="auditForm.content"
              placeholder="请输入附言"></el-input>
          </el-form-item>
        </el-form>
        <div style="text-align:center;">
          <el-button type="primary" @click="postWorkDetail(2)">不通过</el-button>
          <el-button type="primary" @click="postWorkDetail(1)">通过</el-button>
        </div>
      </div>
    </div>
    <span v-if="arrangeType == 'check'" slot="footer" class="dialog-footer">
