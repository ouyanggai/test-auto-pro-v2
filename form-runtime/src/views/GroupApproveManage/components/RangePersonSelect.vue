...
import { deepClone,getObjById,arrayToTree2 } from '@/utils';
export default {
  name: '',
  components: {},
  props: {
  ...
  async created() {
    ...
    this.rangeType = this.nodeAuditScopeList[0]['type'];
    if (this.rangeType == 'personnel') this.allUserList = await this.getCompanyPersonList();
    if (this.rangeType == 'department' || this.rangeType == 'company') {
      if (this.isProject) { // 项目空间，获取项目得列表
        this.getProjectTree();
      } else { // 公司空间
        this.getCompanyTree();
      }
    } else if (this.rangeType == 'role' || this.rangeType == 'position' || this.rangeType == 'personnel') {
      this.getPersonList();
    }
  },
  mounted() {
  ...
  methods: {
    // 审批人范围：角色、岗位、人员类型获取人员
    async getPersonList(){
      this.nodeAuditScopeList.forEach(async item=>{
        if (this.rangeType == 'role') {
          let result = await this.getRoleUserById(item.bizId);
          let newRes = result.map(x=>({
            name:x.userVo.name,
            id:x.userVo.id,
            type: '5',
          }));
          // console.log('newRes',newRes)
          this.treeData.push(...newRes);
          // console.log('treeData',this.treeData)
        } else if (this.rangeType == 'position') {
          ...
          // console.log('newRes',newRes)
          this.treeData.push(...newRes);
        } else if (this.rangeType == 'personnel') {
          let result = this.allUserList.find(x=>x.id == item.bizId);
          result.type = '5';
    ...
    //   for(let i in list){
    //     let item=list[i]
    //     let id = obj[item['id']] ? item['id'] : null;
    //     // console.log('id',id)
    //     if(item['id'] == id)
    //     {
    //       return item
    ...
    //         let value= this.addDepartPerson(item['childrenList'],obj)
    //       //查询到直接返回
    //         if(value){
    //           value['childrenList'] = obj[value['id']]
    //         }
    //       }
    //     }
    //   }
    // },
    // // 用于拼装新的树结构数据(选中的部门范围树结构)
    // getBackNewTreeArray(resData){
    //   let copyTreeData = deepClone(resData);
    //   // console.log('nodeAuditScopeList', this.nodeAuditScopeList)
    //   let allAncestorsList = [];
    //   this.nodeAuditScopeList.forEach(item=>{
    //     let ancestorsList = this.getCheckTag(copyTreeData, item.bizId);
    //     allAncestorsList.push(ancestorsList)
    //   })
    //   let allNeedTreeData = allAncestorsList.flat(1); // 数组扁平化
    //   // console.log('allAncestorsList',allAncestorsList)
    //   console.log('allNeedTreeData',allNeedTreeData)
    //   const obj = {};
    //   const newArr = allNeedTreeData.reduce((item, next) => { // 去重
    //     obj[next.id] ? '' : obj[next.id] = true && item.push(next);
    //     return item;
    //   }, []);
    //   // console.log('newArr',newArr)
    //   // this.treeData = arrayToTree2(newArr);
    //   return arrayToTree2(newArr);
    // },
    getBackNewTreeArray(list, cb) {
      // let ids = ['ede968501ef443efa4e61e0b791f701e', 'fed2ca4ad7554483bc1d09f8d000b4ce']
      return list.reduce((res, item) => {
        if(cb(item)){
          res.push(item)
        } else if(item.childrenList && item.childrenList.length) {
          var childrenList = this.getBackNewTreeArray(item.childrenList, cb);
          if(childrenList.length) res.push({...item, childrenList});
        };
        return res;
      }, []);
    },
      ...
      this.$axios.post(
        ...
        res => {
          ...
          if (res.isSuccess) {
            ...
            let copyFullTreeData = deepClone(res.data);
            let ids = this.nodeAuditScopeList.map(x=>x.bizId);
            // console.log('ids',ids)
            this.treeData = this.getBackNewTreeArray(copyFullTreeData, item => ids.includes(item.id))
            console.log('this.treeData',this.treeData)
            // console.log(222,this.getBackNewTreeArray(copyFullTreeData, item => ids.includes(item.id)))

            // let treeArray = this.getBackNewTreeArray(res.data);
            // let copyFullTreeData = deepClone(res.data);
            // let ids = this.nodeAuditScopeList.map(x=>x.bizId);
            // console.log('ids',ids)
            // console.log(222,this.test(copyFullTreeData, item => ids.includes(item.id)))
            // let currentTargeObj = {};
            // this.nodeAuditScopeList.forEach(item=>{
            //   let currentTarget = getObjById(copyFullTreeData, item.bizId) // 获取审批人范围选定的数据下的人员
            //   currentTargeObj[currentTarget.id] = currentTarget.childrenList.filter(x=>x.type == '5');
            // })
            // this.addDepartPerson(treeArray,currentTargeObj)
