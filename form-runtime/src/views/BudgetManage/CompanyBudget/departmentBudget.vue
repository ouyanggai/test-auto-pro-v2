<template>
  <div class="outer">
    <div class="top">
      ...
      <el-date-picker v-model="query.date" :type="picker['dateType']" :value-format="picker['valueFormat']"
        style="width: 160px;margin-right:5%" :picker-options="expireTimeOption" @change="changeDate" :clearable="false">
      </el-date-picker>
  ...
  methods: {
    ...
    getlistByBudgetId(ids) {
      ...
      this.$axios.post(Api.budgetManage.listByBudgetId, data, res => {
        if (res.isSuccess) {
          let data = res.data
          let departmentBudget = [{departName:'部门',id:'depart'}], projectBudget = [{departName:'项目',id:'project'}]
          for (let key in data) {
