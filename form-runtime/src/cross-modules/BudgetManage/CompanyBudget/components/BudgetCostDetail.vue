<template>
  <div class="outer">
    ...
    <!-- 费用明细列表 -->
    <detailedDialog :detailVisible.sync="detailVisible" v-if="detailVisible" :isShowBudget="false"
                    :detailRow="detailRow" :isShowDepartment="false">
    </detailedDialog>
  </div>
</template>
...
export default {
  ...
  computed: {
    picker() {
      if (this.query.dateType == '1') {
        this.query.date = this.query.date.substr(0, 4);
        return {
          dateType: 'year',
          valueFormat: 'yyyy'
        };
  ...
  beforeRouteEnter(to, from, next) {
    if (from.fullPath == '/groupBudgetManage/companyBudget/index') {
      to.meta.activeMenu = '/groupBudgetManage/companyBudget'
    } else {
  ...
  watch: {
    'query.dateType': {
      handler(a) {
        ...
        //   let curruntMonth = new Date().getMonth() + 1
        //   curruntMonth = curruntMonth >= 10 ? curruntMonth : '0' + curruntMonth
        //   let month = `${this.query.date}-${curruntMonth}`;
        //   this.query.date = `${this.query.date}-${curruntMonth}`;
        //   this.queryMonthData(month);
        // } else {
  ...
  async created() {
    ...
    if (year) this.query.date = year
    let companyId = this.$route.query.companyId
    if (companyId) this.companyId = companyId

    if (ids) {
      this.getDepartmentList().then(() => {
        this.getlistByBudgetId(ids)
      })
    }
  },
  ...
  methods: {
    ...
    getlistByBudgetId(ids) {
      ...
      this.$axios.post(Api.budgetManage.listByBudgetId, data, res => {
        if (res.isSuccess) {
          let data = res.data
          let departmentBudget = [{departName:'部门',id:'depart',sort:0}], projectBudget = [{departName:'项目',id:'project'}]
          for (let key in data) {
            let detailData = data[key]
            detailData.forEach(el => {
              let type = el.type
              let currentTarget = type == 1 ? departmentBudget : projectBudget
              ...
              let sort = el.sort
              if (index == -1) {
                let departmentName = el.budgetTypeVo.departmentName
                let departmentObj = {
                  id: el.departmentId,
                  departName: (el.budgetTypeVo.departmentName == '公司领导' ? '公司固定费用':el.budgetTypeVo.departmentName) || this.departmentList.find(item=>item.id == key)?.name || '',
                  money,
                  costTotal,
                  type,
                  leftMoney,
          ...
          this.tableData = this.tableData.concat(departmentBudget,projectBudget)
    ...
    queryData() {
      if (this.query.depart) {
        let departmentId = this.query.depart;
        this.tableData = [];
        let len = this.orginData.length
        for (let i = 0; i < len; i++) {
          if (this.orginData[i].id == departmentId) {
            this.tableData.push(this.orginData[i]);
          }
        }
      } else {
        this.tableData = deepClone(this.orginData);
      }
    },
    sumNums(values) {
      let val = 0;
