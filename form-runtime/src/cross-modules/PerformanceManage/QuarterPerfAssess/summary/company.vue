  ...
  <div class="performance-dashboard">
    ...
    <div class="header-section">
      <div class="header-info">
        <div class="user-profile">
          <div class="current-date" style="margin-right:10px;">{{ currentDate }}</div>
    ...
    <div class="annual-plan-container" v-if="false">
      <el-card class="section-card">
        <div slot="header" class="section-header" style="justify-content:space-between;">
          <!-- <i class="el-icon-calendar header-icon"></i> -->
          <span class="header-title" style="margin-right:10px">年度工作计划</span>
          <span>
            <el-button type="primary" plain size="mini" @click="_=>_">发 起</el-button>
            <el-button type="info" plain size="mini" @click="_=>_">变 更</el-button>
          </span>
        </div>
    ...
    <div class="middle-content">
      <div class="task-list-container">
        <el-card class="section-card">
          <div slot="header" class="section-header">
            ...
            <span class="header-title" style="margin-right:auto;">绩效评定</span>
             <el-radio-group v-model="radio2" size="mini" style="margin-left:auto">
                <el-radio-button label="待评定">待评定</el-radio-button>
                <el-radio-button label="评定中">评定中</el-radio-button>
                <el-radio-button label="已评定">已评定</el-radio-button>
            </el-radio-group>
            <span style="margin:auto"></span>
          </div>
          <div class="task-list-container-scroll" style="min-height:400px;">
          <div class="task-list">
            <el-table :data="ratingMap[radio2]" class="content-table">
              <el-table-column prop="title" label="分组名称"></el-table-column>
              <el-table-column prop="name" label="时间" min-width="40">
                <template slot-scope="scope">
                  {{ showPlanName(scope.row) }}
                </template>
              </el-table-column>
              <el-table-column prop="updateDate" label="评定时间" min-width="40">
               <template slot-scope="scope">
                  {{ radio2 == '已评定' ? scope.row.updateDate : '-' }}
                </template>
              </el-table-column>
              <!-- <el-table-column prop="user" label="评定人" mwd="120"></el-table-column> -->
              <el-table-column prop="examineStatus" label="评定状态" width="70">
                 <template slot-scope="scope">
                  <el-tag :type="ratingColor[scope.row.examineStatus]">{{ ratingEnum[scope.row.examineStatus] || '-' }}</el-tag>
                </template>
              </el-table-column>
              <el-table-column label="操作" width="70">
                <template slot-scope="scope">
                  <el-button type="text" size="small" @click="viewQuarterPlan(scope.row)" v-if="scope.row.examineStatus != 'not_submitted'">查看</el-button>
                  <el-button type="text" size="small" @click="handleRating(scope.row)" v-if="scope.row.examineStatus == 'not_submitted'">评定</el-button>
                </template>
              </el-table-column>
            </el-table>
          </div>
      ...
      <!-- 绩效趋势图表 -->
      <div class="performance-chart-container" v-if="true">
        <el-card class="section-card">
          <div slot="header" class="section-header">
            <!-- <i class="el-icon-line-chart header-icon"></i> -->
            <span class="header-title" style="margin-right:auto;">绩效分布</span>
            <el-radio-group v-model="radio3" size="mini" style="margin-left:auto">
              <!-- <el-radio-button label="人员视角"></el-radio-button> -->
              <el-radio-button label="公司">公司视角</el-radio-button>
              <el-radio-button label="部门">部门视角</el-radio-button>
            </el-radio-group>
            <span style="margin-left:auto" >
              <el-date-picker v-model="year4" type="year" placeholder="选择年" style="width:99px;margin-right:10px" :clearable="false"
              value-format="yyyy" format="yyyy" @change="handleDistribute"/>
              <el-select ref="quarterPicker" style="width:90px;" placeholder=" " v-model="quarter4" class="font-large" @change="handleDistribute">
          ...
          <div class="chart-wrapper" style="height:400px;">
            <InfiniteScrollTable :tableType="radio3" :tableList="distributeCom" v-show="radio3 == '公司'"></InfiniteScrollTable>
            <InfiniteScrollTable :tableType="radio3" :tableList="distributeDep" v-show="radio3 == '部门'"></InfiniteScrollTable>
             <!-- <el-empty></el-empty> -->
    ...
    <div class="bottom-tables">
      <div class="performance-review-table">
        <el-card class="section-card" style="min-height:400px;">
          <div slot="header" class="section-header">
            <!-- <i class="el-icon-star-off header-icon"></i> -->
            <span class="header-title">计划公示</span>
            <span style="margin-left:auto" >
          ...
          >
            ...
            <el-table-column prop="ratingLevel" label="组织绩效评级" mwd="140"></el-table-column>
            <el-table-column prop="totalScore" label="分数" mwd="80"></el-table-column>
            <el-table-column prop="ratingLevel" label="评级" mwd="80"></el-table-column> -->
          ...
          <div style="display:flex;justify-content:space-between;" v-if="page2.pages > 1">
            <el-button type="text" @click="_=>{fullViewType='publicity';fullViewVisible=true}">查看更多</el-button>
            <el-pagination layout="prev, pager, next" :total="page2.total" style="float:right;" :page-size="page2.size" :current-page="page2.current"
      ...
      <div class="quarter-plan-table">
        <el-card class="section-card" style="min-height:400px;">
          <div slot="header" class="section-header">
            ...
            <span class="header-title">工作计划</span>
            <span style="margin-left:auto" >
          ...
          <div style="display:flex;justify-content:space-between;" v-if="page1.pages > 1">
            <el-button type="text" @click="_=>{fullViewType='plan';fullViewVisible=true}">查看更多</el-button>
            <el-pagination layout="prev, pager, next" :total="page1.total" style="float:right;" :page-size="page1.size" :current-page="page1.current"
             @size-change="handleSizeChange" @current-change="handleCurrentChange"></el-pagination>
          </div>
        </el-card>
      </div>
    </div>
    <div class="quarter-plan-table" style="margin-top: 10px;">
      <el-card class="section-card" style="min-height:320px;">
        ...
        <div style="display:flex;justify-content:space-between;" v-if="page3.pages > 1">
            <el-button type="text" @click="_=>{fullViewType='performance';fullViewVisible=true}">查看更多</el-button>
            <el-pagination layout="prev, pager, next" :total="page3.total" style="float:right;" :page-size="page3.size" :current-page="page3.current"
    ...
    <announcement-dialog :visible.sync="dialogVisible" :type="type" :id="currentData.id" :companyOptions="companyOptions" @confirm="getPublicityList"/>
    <fullView-dialog v-if="fullViewVisible" :visible.sync="fullViewVisible" :type="fullViewType"></fullView-dialog>
  </div>
</template>
...
import fullViewDialog from './fullViewDialog.vue'
...
import AnnouncementDialog from '@/views/PerformanceManage/QuarterPerfAssess/announcement/components/announcementDialog.vue';
...
export default {
  ...
  components: {chooseFlowDialog, FlowDialog, personDetail, AddTaskArrangeDialog, InfiniteScrollTable, AnnouncementDialog, fullViewDialog},
  data() {
    return {
      fullViewType: 'publicity', // 'plan publicity performance',
      ...
      // 操作按钮
      actions: [
        { label: '发 起', type: 'primary', auditWay: "kpi2_appraise" },
        { label: '变 更', type: 'info', auditWay: "" }
      ],
      // 任务列表
      taskList: [
        {
  ...
  mounted() {
    ...
    this.$bus.$on('kpiRatingSaveDraftSuccess', _ => {
  ...
  beforeDestroy() {
    ...
    this.$bus.$off('kpiRatingSaveDraftSuccess');
  },
  watch: {
    ...
    approveDialogVisible(val) {
      if (!val) {
        this.init();
        setTimeout(_ => {
          this.getRatingList('not_submitted');
          this.getRatingList('under_review');
          this.getRatingList('pass');
        }, 1000);
      }
    },
  },
  methods: {
    exportTableData(exportCycle) {
      var param = this.getPerformance();
      param.pagination = false;
      this.$axios.post(
        '/web/plan/api/kpi2Group/export',
        param,
        res => {
          if (res) {
    ...
    handleRating(row) {
      sessionStorage.setItem('currentKpiRatingRow', JSON.stringify(row));
      this.flowType = 'kpi2_rating_level_evaluate';
      this.getFlowByTemplate('kpi2_rating_level_evaluate');
    },
    ...
    handleDistribute() {
      this.getDistribute('my_company');
      this.getDistribute('dep');
    },
    getDistribute(kpiScope) {
      this.$axios.post('/web/plan/api/kpi2Group/getKpi2Ledger',
        {
          data: {
            kpiScope,
            targetTime: this.year4 ? this.year4 + '' + this.quarter4 : this.formData.targetTime, // this.formData.year + '' + this.formData.quarter
          }
        },
        res => {
          if (res.isSuccess) {
            if (kpiScope == 'my_company') {
              this.distributeCom = res.data || [];
            } else if (kpiScope == 'dep') {
              this.distributeDep = res.data || [];
            }
          }
        });
    },
    getRatingList(examineStatus) {
      this.$axios.post('/web/plan/api/kpi2UserGroupSummary/list',
        {
          data: {
            // targetTime: this.formData.year + '',
            examineStatus
          }
        },
        res => {
          ...
          if (examineStatus == 'not_submitted') {
            this.ratingMap['待评定'] = res.data || [];
          } else if (examineStatus == 'under_review') {
            this.ratingMap['评定中'] = res.data || [];
          } else if (examineStatus == 'pass') {
            this.ratingMap['已评定'] = res.data || [];
          }
        });
    },
    getUserInfo() {
    ...
    getQuarterPlans() {
      ...
      const param = {
        data: {
          ...
          targetTime: this.year2 ? this.year2 + '' + this.quarter2 : this.formData.targetTime,
    ...
    getPerformance() {
      const param = {
        data: {
          kpiScope: 'my_company',
          targetTime: this.year3 ? this.year3 + '' + this.quarter3 : this.formData.targetTime,
    ...
    getPublicityList() {
      ...
      //   data: {
      //     kpiScope: 'personal',
      //     targetTime: this.this.formData.year + ''
      //   },
      //   pagination: false,
      // };
      // this.$axios.post('/web/plan/api/kpi2Group/list', param,
      ...
      this.$axios.post(Api.newPerformance.workPlanPromulgateList,
        {
          data: {
            companyId: '',
            noticeStatus: null,
            targetTime: this.year1 ? this.year1 : this.formData.year + '', // new Date().getFullYear() + ''
          },
          pagination: true,
...
::v-deep .reset-padding-dialog.is-fullscreen {
  .el-dialog__body {
...
::v-deep .task-list-container .el-card__body {
  padding: 0px 6px;
}
...
::v-deep .performance-chart-container .el-card__body {
  padding: 0px 6px;
}
