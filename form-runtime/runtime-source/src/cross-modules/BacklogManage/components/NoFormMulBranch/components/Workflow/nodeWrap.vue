      ...
      >
        ...
        <div>
          ...
          <div class="content" @click="setPerson(1)">
            <!-- @click="handleNode(1)" -->
            <div class="text">
              <span v-if="nodeConfig.type == 'start'">姓名：{{ initiator }}</span>
              <span v-else>定义节点配置</span>
              <span
                v-if="nodeConfig.type != 'start' && (nodeConfig.flowNodeAuditConfig.flowNodeDetailConfigList.length ||
        ...
        <div>
          ...
          <div class="content" @click="setPerson(1)">
            <div class="text">
              <span>定义节点配置</span>
              <span
                v-if="nodeConfig.flowNodeAuditConfig.flowNodeDetailConfigList.length || (nodeConfig.flowNodeAuditConfig.auditType && nodeConfig.flowNodeAuditConfig.auditType != 'assign')"
                class="complete"
              ><i class="el-icon-connection" /></span>
            </div>
            <i class="anticon anticon-right arrow" />
        ...
        <div class="condition_content drawer_content">
          <div class="approver_content">
            <el-form ref="nodeForm" :model="nodeForm" :rules="nodeFormRules">
              ...
              >
                <el-input-number
                  v-model="nodeForm.countersignNum"
                  :min="1"
                  step-strictly
                />
              </el-form-item>
              ...
              <el-form-item label="审批人设置：" :label-width="formLabelWidth">
                <el-radio-group v-model="nodeForm.auditType" class="clear">
                  ...
                  <!-- <el-radio
                    v-if="nodeForm.type == 'scramble' && nodeConfig.type != 'start'"
                    :label="'department_supervisor'"
                  >发起人部门主管</el-radio>
                  <el-radio
                    v-if="nodeForm.type == 'scramble' && nodeConfig.type != 'start'"
                    :label="'branched_passage_manager'"
                  >发起人分管副总</el-radio> -->
                  <!-- <el-radio
                    v-if="nodeForm.type == 'scramble' && nodeConfig.type != 'start'"
                    :label="'position'"
              ...
              >
                ...
                <div v-if="nodeForm.rangeType == 'position' || nodeForm.rangeType == 'role'" style="max-height: 250px;overflow: auto;">
                  <el-collapse v-for="duty in nodeForm.personRangeTagList" :key="duty.id">
                    <el-collapse-item v-if="nodeForm.rangeType == 'position'" :title="'岗位：'+ duty.name" :name="1">
...
// import JobTitleSelectDialog from './components/JobTitleSelectDialog';
// import JobPosition from './components/JobPosition.vue';
// import SelectRangeDialog from './components/selectRangeDialog.vue';
// import FieldSelectDialog from './components/FieldSelectDialog';
