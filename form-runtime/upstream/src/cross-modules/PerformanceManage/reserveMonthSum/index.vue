  ...
  <div class='app'>
    ...
    <el-dialog custom-class="monthlyPerfBlocSum" v-if="visible" title="" :fullscreen="true" :visible="visible" :append-to-body="true" :close-on-click-modal="false"  center
      ...
      <div ref="print" class="print">
        <reserveMonthSum :actionType="'preview'" :bizId="bizId"></reserveMonthSum>
      </div>
    </el-dialog>
...
import reserveMonthSum from './reserveMonthSum.vue';
...
export default {
  name: '',
  components: { DyTable, reserveMonthSum },
  data () {
    return {
      selectRow: null,
      treeData: [],
      ...
      colKey2: {
        ...
        department: {
          label: '部门',
          handle: (scope, createElement) => {
            return <span>{scope.row.department.departmentName}</span>;
          }
        },
        duty: {
          label: '岗位',
          handle: (scope, createElement) => {
            return <span>{scope.row.duty.dutyName}</span>;
          }
        }
      },
      colKey: {
        ...
        //   label: '公司',
        //   handle: (scope, createElement) => {
        //     return <span>{scope.row.user?.company?.name}</span>;
        //   }
        // },
        // department: {
        //   label: '部门',
        //   handle: (scope, createElement) => {
        //     return <span>{scope.row.user?.department?.departmentName}</span>;
        //   }
        // },
        // duty: {
        //   label: '岗位',
        //   handle: (scope, createElement) => {
        //     return <span>{scope.row.user?.duty?.dutyName}</span>;
        //   }
        // },
        initiator: {
          label: '发起人',
      ...
      actions: [
        ...
        },
        {
          ...
          handle: (scope, createElement, self) => {
            ...
            if (scope.row.examineStatus == 'not_submitted') {
  ...
  methods: {
    deleteItem(row) {
      this.$confirm('确定删除?', '提示', {
        ...
        .then(() => {
          this.$axios.post(
            '/web/plan/api/kpiSummary/deleteReserveKpiSummary',
            { data: { id: row.id }},
            (res) => {
              if (res.isSuccess) {
                this.getList();
    ...
    sortDelete(row) {
      ...
      this.$confirm('确定删除选中人员?', '提示', {
        ...
        .then(() => {
          ...
          this.sortTableData.splice(this.selectRow.sortIndex, 1);
          this.selectRow = null;
          this.clickSetSort(true, _ => {
            this.fetchSortData();
          });
    ...
    getList() {
      ...
      this.$axios.post(
        '/web/plan/api/kpiSummary/findReserveKpiSummaryList',
        params,
        (res) => {
          if (res.isSuccess) {
