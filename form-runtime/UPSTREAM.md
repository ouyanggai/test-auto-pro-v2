...
- 规范分支：`test`
- 当前运行快照基线：`bff4ef8b938db5578c3f7eab1f482a4e9388917c`（历史 `master` 快照；切换规范分支不自动覆盖）
...
参考仓库只能先由 `make refs-sync`/`make refs-status` 保证规范远端、`test` 和干净工作树。`test` 对应当前目标测试环境并包含 `GroupApproveManage` 的测试环境同步结果。维护 API 不接受任意路径、分支、HEAD 或命令；每个任务把创建时的当前 HEAD 持久化，Worker 在同步前复核同一快照。参考仓库后续安全快进不需要修改代码中的历史 HEAD。
